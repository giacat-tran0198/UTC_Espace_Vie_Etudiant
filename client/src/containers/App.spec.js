...
apiCall.getUser = jest.fn().mockResolvedValue({
    data: {
        id: 1,
        username: "user1",
...
describe("App", () => {
    ...
    it("displays My Profile on TopBar after signup success", async () => {
        const {queryByPlaceholderText, container} = setup("/signup");
    ...
    it("saves logged in user data to localStorage after login success", async () => {
        ...
        const dataInStorage = JSON.parse(localStorage.getItem("blog-auth"));
